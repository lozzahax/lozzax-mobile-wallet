...
#include <unistd.h>
#include <thread>
// #include "LozzaxWalletListener.h"
#ifdef ANDROID
#include "../External/android/lozzax/include/wallet2_api.h"
#else
#include "../External/ios/lozzax/include/wallet2_api.h"
#endif

namespace Lozzax = Wallet;
...
extern "C"
{
    ...
    struct LozzaxWalletListener : Lozzax::WalletListener
    {
        uint64_t m_height;
        bool m_need_to_refresh;
        bool m_new_transaction;

        LozzaxWalletListener()
        {
            m_height = 0;
    ...
    struct TransactionInfoRow
    {
        ...
        TransactionInfoRow(Lozzax::TransactionInfo *transaction)
        {
    ...
    struct PendingTransactionRaw
    {
        ...
        uint64_t fee;
        char *hash;
        Lozzax::PendingTransaction *transaction;

        PendingTransactionRaw(Lozzax::PendingTransaction *_transaction)
        {
    ...
    struct StakeUnlockResult
    {
      bool success;
      char *msg;
      Lozzax::PendingTransaction *pendingTransaction;

      StakeUnlockResult(bool _success, char *_msg,  Lozzax::PendingTransaction *_pendingTransaction)
      {
        success = _success;
        msg = _msg;
    ...
    Lozzax::Wallet *m_wallet;
    Lozzax::TransactionHistory *m_transaction_history;
    LozzaxWalletListener *m_listener;
    Lozzax::Subaddress *m_subaddress;
    Lozzax::SubaddressAccount *m_account;
    uint64_t m_last_known_wallet_height;
    uint64_t m_cached_syncing_blockchain_height = 0;
    std::mutex store_mutex;


    void change_current_wallet(Lozzax::Wallet *wallet)
    {
        m_wallet = wallet;
        m_listener = nullptr;
    ...
    EXPORT
    Lozzax::Wallet *get_current_wallet()
    {
        return m_wallet;
    }
    ...
    {
        Lozzax::NetworkType _networkType = static_cast<Lozzax::NetworkType>(networkType);
        Lozzax::WalletManagerBase *walletManager = Lozzax::WalletManagerFactory::getWalletManager();
        Lozzax::Wallet *wallet = walletManager->createWallet(path, password, language, _networkType);

        auto stat = wallet->status();

        auto& [status, errorString] = stat;

        if (status != Lozzax::Wallet::Status_Ok)
        {
    ...
    {
        Lozzax::NetworkType _networkType = static_cast<Lozzax::NetworkType>(networkType);
        Lozzax::Wallet *wallet = Lozzax::WalletManagerFactory::getWalletManager()->recoveryWallet(
            std::string(path),
            std::string(password),
            std::string(seed),
            _networkType,
    ...
    {
        Lozzax::NetworkType _networkType = static_cast<Lozzax::NetworkType>(networkType);
        Lozzax::Wallet *wallet = Lozzax::WalletManagerFactory::getWalletManager()->createWalletFromKeys(
            std::string(path),
            std::string(password),
            std::string(language),
            _networkType,
        ...
        if (status != Lozzax::Wallet::Status_Ok || !errorString.empty())
        {
            error = strdup(errorString.c_str());
            return false;
        }
    ...
    {
        nice(19);
        Lozzax::NetworkType networkType = static_cast<Lozzax::NetworkType>(nettype);
        Lozzax::Wallet *wallet = Lozzax::WalletManagerFactory::getWalletManager()->openWallet(std::string(path), std::string(password), networkType);
        change_current_wallet(wallet);
    }
    ...
    {
        return Lozzax::WalletManagerFactory::getWalletManager()->walletExists(std::string(path));
    }

    EXPORT
    void close_current_wallet()
    {
        Lozzax::WalletManagerFactory::getWalletManager()->closeWallet(get_current_wallet());
    ...
    {
        nice(19);
        Lozzax::Wallet *wallet = get_current_wallet();
    ...
    bool stake_create(char *service_node_key, char *amount, Utf8Box &error, PendingTransactionRaw &pendingTransaction)
    {
        ...
        uint64_t _amount = Lozzax::Wallet::amountFromString(std::string(amount));
        transaction = m_wallet->stakePending(std::string(service_node_key), _amount);

        int status = transaction->status().first;

        if (status == Lozzax::PendingTransaction::Status::Status_Error || status == Lozzax::PendingTransaction::Status::Status_Critical)
        {
    ...
    {
        std::unique_ptr<Lozzax::StakeUnlockResult> stakeUnlockResult{m_wallet->canRequestStakeUnlock(service_node_key)};
    ...
    {
        std::unique_ptr<Lozzax::StakeUnlockResult> stakeUnlockResult{m_wallet->requestStakeUnlock(service_node_key)};
    ...
    void set_listener()
    {
        ...
        m_listener = new LozzaxWalletListener();
    ...
    {
        std::vector<Lozzax::SubaddressRow *> _subaddresses = m_subaddress->getAll();
        size_t size = _subaddresses.size();
        int64_t *subaddresses = (int64_t *)malloc(size * sizeof(int64_t));

        for (int i = 0; i < size; i++)
        {
            Lozzax::SubaddressRow *row = _subaddresses[i];
    ...
    {
        std::vector<Lozzax::SubaddressAccountRow *> _accounts = m_account->getAll();
        size_t size = _accounts.size();
        int64_t *accounts = (int64_t *)malloc(size * sizeof(int64_t));

        for (int i = 0; i < size; i++)
        {
            Lozzax::SubaddressAccountRow *row = _accounts[i];
    ...
    {
        std::vector<Lozzax::TransactionInfo *> transactions = m_transaction_history->getAll();
        size_t size = transactions.size();
        int64_t *transactionAddresses = (int64_t *)malloc(size * sizeof(int64_t));

        for (int i = 0; i < size; i++)
        {
            Lozzax::TransactionInfo *row = transactions[i];
    ...
    {
        Lozzax::Utils::onStartup();
        Lozzax::WalletManagerFactory::setLogLevel(0);
    }
